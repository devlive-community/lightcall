...
!!! danger "注意"
    ...
    LightCall 需要在方法上添加 `@Put` 注解来标识该方法是一个 HTTP PUT 请求。并且添加 `@Put` 注解的类必须是一个接口。

!!!

我们使用的模拟数据是，他的代码可以在 [这里](https://github.com/devlive-community/lightcall/blob/dev/src/test/java/org/devlive/lightcall/example/put/PutService.java "PutService" "_blank") 查看。
